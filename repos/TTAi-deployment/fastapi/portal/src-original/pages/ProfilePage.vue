...
<template>
  <section class="single-column-grid profile-shell-page">
    ...
    <div v-if="loading" class="empty-state">Loading profileâ€¦</div>
      ...
      <article class="panel-subtle">
        <form class="auth-form" @submit.prevent="handleSave">
          <label>
            ...
            <input v-model="form.name" type="text" :placeholder="$t('ProfilePage.yourName')" />
          </label>
          <label>
            <span>{{ $t('ProfilePage.email') }}</span>
            <input v-model="form.email" type="email" :placeholder="$t('ProfilePage.youcompanycom')" />
          </label>
